...
const searchPatient = (event: any) => {
    if(event.length > 1){
        axios.get(route('search-patient'), {
            params: {query: event}
        }).then(({data}) => {
...
<template>
    <div class="flex flex-col gap-6 overflow-y-auto px-2 py-4">
        <div class="flex flex-col gap-4">
            ...
            <VueMultiselect id="patient_id" :placeholder="trans('Select')" v-model="selectedPatient" :options="patients" @search-change="searchPatient" :custom-label="nameIIN">
        ...
        <div class="flex flex-col gap-4">
            ...
            <VueMultiselect id="doctor_id" :placeholder="trans('Select')" v-model="selectedDoctor" :options="doctors" @search-change="searchDoctor" label="full_name">
